...
# Penjelasan mengenai Primary Key dan Foreign key
...
# Basic Operation in Relational Database
CREATE TABLE users (
...
INSERT into users
VALUES 
(NULL, 'test03', 'test03@gmail.com','2021-10-10')
;

DELETE FROM Mahasiswa.users
WHERE user_id=2;
...
UPDATE users set
email = 'bagas007@mail.com',
username = 'bagasprakasa'
...
# Other Operation in Relational Database
...
SELECT MIN(population), MAX(population), AVG(population) FROM states s ;

SELECT GROUP_CONCAT(name SEPARATOR ','), join_year FROM  states GROUP BY join_year ;

SELECT SUM(population) AS USA_POPULATION FROM states s ;
...
SELECT customers.customer_id
	...
	,orders.order_id
	,orders.amount 
FROM customers inner join orders where customers.customer_id = orders.customer_id;
...
-- EXAMPLE 2: LOAD SCHEMA
-- ONLINE ERD TOOLS: https://dbdiagram.io/d
